...
8. **Agentic Card (added 2026-09-05, see D6.2).** The card from the concept section becomes a product: a rule-driven purchase agent that lives *inside* the policy engine. `services/agent_card_service.py` (deterministic, no model in the loop): a SYNTHETIC two-platform price feed with a bounded random walk; `PurchaseRule` (product, target price, platforms, compound conditions, manual/auto approval, 15-min window); a monitor loop in `main.py` (`PRICE_TICK_SECONDS`) that re-quotes, evaluates every active rule with a written `last_eval` (price check + each condition met/not-met), and when all hold creates a PURCHASE intent through `money_action_service.create()` — ALLOW → in-app notification, YES → Razorpay test checkout; REQUIRE_APPROVAL → notification with `Approval.expires_at`; DENY → rule BLOCKED with the engine's reason. Lapsed window → `expire_unexecuted()` (new ALLOWED/APPROVED → CLOSED edge), hold released, rule keeps watching under a cooldown. Auto mode settles only on ALLOW and only with DEBUG, via `settle_success` with `simulated` evidence. The card's limits ARE `SpendPolicy` (monthly cap / per-purchase cap / ask-me-above / frozen) — `PATCH /api/card/{u}/limits` edits the policy everything else enforces, audited before/after. Notifications are in-app only (`Notification`, bell + list, YES/NO). Model tools: `get_agent_card` (read-only) and `create_purchase_rule` (gated by amount provenance, write provenance and the taint lock, like a money tool). Routes in `api/card.py`, DEBUG levers `POST /debug/card/price/{product}` and `/debug/card/rules/{rule}/expire-now`; 32 tests in `tests/test_agent_card.py`.
...
### Phase 8 — what was actually built *(2026-09-05)*

All eight items done; `backend/tests/test_hardening.py` (22 tests) and `backend/tests/test_degradation.py` (15) assert them rather than asserting they exist.

1. **Rate limiting** — `slowapi`. 240/min per IP by default (the browser's own polling must not be throttled — pinned by a test), **12/min per USER on `/api/chat`** keyed on `X-User-Id` with an IP fallback that is never more permissive, 20/min on `/api/intents/{id}/execute`, webhook exempt (Razorpay retries from its own range; signature verification is the stronger control). 429 carries `Retry-After` and says *nothing was executed*. `headers_enabled=False` deliberately: with it on, slowapi requires every limited handler to take a `response` parameter it never uses.
2. **request_id** — a `ContextVar`, so a log line written deep in a service carries it without threading a parameter through every signature. Honours a caller-supplied `X-Request-ID`, echoes it on every response. `JSON_LOGS=true` switches to machine-readable output; human-readable is the default because JSON on a demo laptop is strictly worse.
3. **`/metrics`** — HTTP requests/duration by ROUTE TEMPLATE (a label per user id would blow up cardinality and leak ids), policy verdicts by decision **and rule**, intent transitions counted in `transition()` (the only place a status is ever assigned, so none can escape), webhook validity, agent turns, plus an `audit_chain_ok` gauge refreshed per scrape. Counters are wired inside `_deny/_allow/_require_approval` so a future rule cannot silently skip the metric, and every counter is in a try/except: a metric must never break the thing it measures.
4. **`/health` vs `/health/ready`** — liveness touches nothing, so it cannot fail for someone else's reasons. Readiness checks DB, Ollama (reachable **and** the configured model actually pulled — otherwise that surfaces as a confusing error on the first chat) and Razorpay config. **Only the database earns 503**; a missing model is `degraded`, because claiming otherwise would contradict the product's central claim.
5. **CI** — `.github/workflows/ci.yml`. Secret hygiene runs first and blocks everything: a key-**shaped** `rzp_live_…` match over tracked files (matching the bare token would fail the build on the docs describing the check), `.env` absent from the index **and from history**, and the `rzp_test_` startup guard still present. Then pytest on 3.10 + 3.13, the benchmark (non-zero exit if a PRD §6.1 target slips, metrics uploaded as an artifact), ruff blocking, mypy and pip-audit advisory.
6. **Backup + rehearsed restore** — `scripts/backup_db.py` using SQLite's online `Connection.backup()`, not a file copy (a `cp` of a live file can capture a torn page, and under WAL it misses the `-wal` commits entirely). `verify` runs `integrity_check` **and** the app's own chain verification; `rehearse` runs backup → verify → restore into a throwaway and compares row counts. Writing the tests found three real bugs in the script: `verify()` crashed on a non-database (the exact input it exists to detect), `_counts()` raised while trying to describe a corrupt file, and `restore()` could not write over a corrupt destination — which is the situation restore exists for.
7. **SQLite tuning** — `journal_mode=WAL` + `synchronous=NORMAL` on file databases, skipped for in-memory (where the pragma is meaningless). WAL is the real fix here: this app writes on two timers while the browser polls, and in rollback-journal mode those readers queued behind a background write for no reason. `busy_timeout=15000` was already in place from the 2026-09-04 OneDrive lock incident.
8. **Degradation matrix** — `docs/degradation_matrix.md`, every row enforced by a test. Writing it found that the UI's FAQ claimed *"the outage itself is recorded in the audit trail"* while the code only logged it; `_degraded_reply()` now audits `degraded_reply:{cause}`. A documented claim the code does not honour is worse than no claim.
...
### D6.2 — Agentic Card: rules the card executes, limits the card *is* *(resolved 2026-09-05, Phase 6)*

**Context:** the product owner's workflow capsule (`campus_hood_agent_workflow.md`): student sets a rule → agent polls prices → all compound conditions met → soft-lock + notify → YES (or auto) → virtual card authorises within caps → charge → ledger → rule done. Technical details were left open; the core loop was not.

**Chosen:** (1) **No second set of limits.** The card's monthly cap, per-purchase cap, approval line and frozen switch are the existing `SpendPolicy` fields — the same ones `policy_engine` checks for every purchase from any source. A card that could be more permissive than the policy would be a second policy. (2) **Deterministic loop, model outside it.** Price feed, rule evaluation, firing, expiry are code; the model gets a read-only view (`get_agent_card`) and may create a rule only from the user's own words and numbers (`create_purchase_rule`, gated like a money tool). (3) **Synthetic prices.** No Flipkart/Amazon keys exist and PRD s11 forbids scraped merchant data; two obviously-fake platforms, whole-rupee quotes bounded to 60–100% of MRP, DEBUG lever to pin a price for demos. (4) **Manual mode pays through Razorpay test checkout; auto mode is DEBUG-only simulated settlement** on ALLOW, and can never skip the approval line. (5) **In-app notifications only** for now; the `Notification` table is the seam for push/SMS later. (6) **15-minute window** (the doc's value) with a 10-minute re-fire cooldown after NO/lapse so a low price does not nag every tick.

**Not chosen — letting a rule bypass the approval threshold in auto mode:** the doc's `approval_mode: auto` would make a standing instruction outrank the user's own "ask me above ₹500" line. Here auto mode only removes the tap for purchases the policy already allows without one.
